    ...
    <div className="flex-1 overflow-y-auto h-full">
      <div className="max-w-2xl mx-auto px-8 py-12">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl font-bold font-prose text-slate-900 mb-3">
            {activeBook?.title || 'Untitled'}
          </h1>
          {activeBook?.author && (
            <p className="text-slate-500 font-prose text-lg">{activeBook.author}</p>
          )}
        </div>
        ...
        {items.map((item) => {
          if (item.type === 'break') {
            return (
              <div key={item.id} className="text-center my-8 text-slate-400 text-lg tracking-widest">
                {item.content || '* * *'}
              </div>
            );
          }
          if (item.type === 'frontmatter') {
            return (
              <div key={item.id} className="mb-8">
                {item.customTitle && (
                  <h2 className="text-xl font-semibold font-prose text-slate-800 mb-4">{item.customTitle}</h2>
                )}
                {item.content && (
                  <div
                    className="prose prose-slate max-w-none font-prose"
          ...
          return (
            <div key={item.id} className="mb-12">
              <h2 className="text-2xl font-bold font-prose text-slate-800 mb-6 pb-2 border-b border-slate-200">
                {node.title}
              </h2>
              {html ? (
                <div
                  className="prose prose-slate max-w-none font-prose prose-p:leading-relaxed"
