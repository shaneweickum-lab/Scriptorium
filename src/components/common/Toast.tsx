...
// How long before the store removes the toast we start fading it out.
// Must be ≤ the store's own removal timeout (3 000 ms / 5 000 ms).
const EXIT_LEAD_MS = 350;
const DURATION_NORMAL      = 3_000;
const DURATION_ACHIEVEMENT = 5_000;
...
/** Individual toast — owns its own enter/exit animation state. */
function ToastItem({ toast, onRemove }: { toast: ToastData; onRemove: (id: string) => void }) {
  const [exiting, setExiting] = useState(false);
  const isAchievement = toast.type === 'achievement';
  const totalDuration = isAchievement ? DURATION_ACHIEVEMENT : DURATION_NORMAL;

  // Schedule the exit animation to start just before the store removes the toast
  useEffect(() => {
    const t = setTimeout(() => setExiting(true), totalDuration - EXIT_LEAD_MS);
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg border ${
        exiting ? 'wp-toast-exit' : 'wp-toast-enter'
      }`}
      style={
        isAchievement
          ? {
    ...
    >
      ...
      <button
        ...
        className="text-slate-400 hover:text-slate-200 ml-1 shrink-0 transition-colors"
      >
        <X size={14} />
      </button>
    </div>
