  ...
  return (
    ...
    >
      ...
      <p className={`text-[11px] italic mb-2 leading-relaxed line-clamp-2 ${
        comment.resolved ? 'text-slate-400' : 'text-amber-600'
      }`}>
        "{comment.quote}"
      </p>
      ...
      {editing ? (
        <div className="flex flex-col gap-1.5" onClick={(e) => e.stopPropagation()}>
          <textarea
            ...
            className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-800 resize-none focus:outline-none focus:border-violet-400"
            rows={3}
          />
          <div className="flex gap-1.5">
            <button
              onClick={handleSaveEdit}
              className="px-2.5 py-1 rounded-lg text-white text-xs font-medium transition-opacity hover:opacity-90"
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors"
            >
              Cancel
            </button>
          </div>
...
export function CommentsPanel({ editor, nodeId, focusedId, onFocusChange, draft, onDraftCancel, onClose }: Props) {
  ...
  return (
    <div className="flex flex-col h-full w-72 border-l border-slate-200 bg-white shrink-0">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        ...
        {draft && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 flex flex-col gap-2">
            <p className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">New comment</p>
            <p className="text-[11px] italic text-amber-600 leading-relaxed line-clamp-2">
              "{draft.quote}"
            </p>
            <textarea
              ref={draftInputRef}
              ...
              className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-2 text-xs text-slate-800 placeholder-slate-400 resize-none focus:outline-none focus:border-amber-400"
              rows={3}
            />
            <div className="flex gap-1.5">
              <button
                ...
                className="flex-1 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 disabled:opacity-30 text-white text-xs font-semibold transition-colors"
              >
                Add Comment
              </button>
              <button
                onClick={onDraftCancel}
                className="px-3 py-1.5 rounded-lg text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        {active.length === 0 && !draft ? (
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
            ...
            <p className="text-[10px] text-slate-400">Select text and click 💬 to add one</p>
          </div>
        ) : (
        ...
        {resolved.length > 0 && (
          <div className="flex flex-col gap-2">
            <button
              ...
              className="flex items-center gap-1.5 text-[10px] text-slate-400 hover:text-slate-600 uppercase tracking-wider transition-colors"
            >
